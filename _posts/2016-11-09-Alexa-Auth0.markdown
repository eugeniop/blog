...
> This 2 series posts are largely based on a proof of concept project developed by my friend Pushp Abrol for a customer. Thanks Pushp!
...
If all works well, you will see the same JSON object we defined in the API. And clicking on "Listen" will let you hear exactly what the Echo would respond to you.

The object that Alexa POSTs to the API will be on the left pane:
...
At this point everything is ready for a real test run. Just enable the skill in your mobile Alexa app.

## Security

At this stage, there's no security (or a very weak one). We need to turn on security on the API level, and only accept requests from a trusted client. In this architecture, the client of the API is Alexa itself.

Fortunately for me, Amazon designed Alexa in such a way, that 3rd party security is easily enabled. And that will be the main focus for Part II. 
